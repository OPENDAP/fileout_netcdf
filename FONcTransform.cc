...
void FONcTransform::transform()
{
    ...
    BESDEBUG( "fonc_detail", *this << endl );

    // Open the file for writing
    // I tried using NC_64BIT_OFFSET as a fix for ticket 1906, but it had no efffect.
    // jhrg 5/11/12
    int stax = nc_create(_localfile.c_str(), NC_CLOBBER /*| NC_64BIT_OFFSET*/, &_ncid);
    if (stax != NC_NOERR) {
    ...
    try {
        ...
        BESDEBUG( "fonc", "Entering define mode" << ", _ncid: " << _ncid << endl );
        int status = nc_redef(_ncid);
        if (status != NC_NOERR) {
            string err = (string) "File out netcdf, " + "Failed to enter define mode (" + _localfile + ").";
            FONcUtils::handle_error(stax, err, __FILE__, __LINE__);
        }
        ...
        BESDEBUG( "fonc_detail", "Adding Global Attributes" << endl
                << globals << endl );
        ...
        BESDEBUG( "fonc", "Leaving define mode" << ", _ncid: " << _ncid << endl);
        status = nc_enddef(_ncid);
        if (status != NC_NOERR) {
            string err = "fileout.netcdf - Failed to end definition phase  (error code: " + long_to_string(status)  + ")";
            FONcUtils::handle_error(stax, err, __FILE__, __LINE__);
        }
