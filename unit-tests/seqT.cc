...
public:
    MySequence(const string &n, const string &d) : Sequence(n,d) {}
    MySequence(const MySequence &rhs) : Sequence(rhs) {}
    virtual ~MySequence() {}
    ...
    virtual BaseType *ptr_duplicate() { return new MySequence(*this); }
    virtual bool read() { set_read_p( true ); return false ; }
};
